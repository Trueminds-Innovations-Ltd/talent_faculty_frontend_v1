...
const supportRows: SupportRow[] = [
  ...
  },
  {
    learner: {
      ...
      email: "aisha.andersson40@gcschool.edu",
    },
    module: "User Research & Discovery",
    grade: 30,
    progress: 15,
...
const AssignmentsTable: React.FC = () => (
  <table className="w-full min-w-[640px] border-collapse">
    <TableHead
      cols={["Learners", "Modules", "Assignment", "Due", "Grade", "Status"]}
    />
    <tbody>
    ...
    <tbody>
      {supportRows.map((row) => (
        ...
        >
          ...
          <td className="py-3 pr-4 text-sm text-gray-600">{row.module}</td>
          <td className="py-3 pr-4 text-sm font-medium text-rose-500">
            {row.grade}
          </td>
          <td className="py-3 pr-4">
            <ProgressBar value={row.progress} />
        ...
        <div className="mb-5 flex w-fit gap-1 rounded-full bg-gray-50 p-1 text-sm">
          {TABS.map((tab) => (
            <button
              ...
              className={`whitespace-nowrap rounded-full px-4 py-1.5 font-medium transition ${
                activeTab === tab.key
