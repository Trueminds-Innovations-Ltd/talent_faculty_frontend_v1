...
export default function SignupEmail() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response?.success !== false) {
        navigate('/signup-verify-email', { state: { email: trimmedEmail } })
      } else {
      ...
      setError(apiErr?.details || apiErr?.message || 'Failed to send verification code. Please try again.')
    } finally {
      setIsLoading(false)
    }
  }
  ...
  return (
    <div className="flex flex-col lg:flex-row min-h-screen font-sans">
      ...
      {/* Right Panel - Form */}
      <div className="flex-1 flex flex-col items-center md:justify-center bg-[#E6F4EA] px-6 py-12 sm:px-12 overflow-y-auto">
