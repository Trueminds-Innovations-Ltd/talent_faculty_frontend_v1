...
const CourseListItem: React.FC<CourseListItemProps> = ({ title, instructor, progress, onResume, totalLectures, completedLectures }) => {
  return (
    <div className="flex items-center gap-8 py-4 border-b border-neutral-50 last:border-0 hover:bg-neutral-50/50 transition-colors rounded-lg px-2 -mx-2">
      <div className=" flex gap-[24px] h-16 w-24 rounded-lg flex-shrink-0 w-[312px]">
        <img src="./course-image.jpg" alt="" className="rounded-[8px]"/>
      ...
      { progress === 100 ?<button className="text-sm font-semibold text-primary hover:text-[#057834] whitespace-nowrap transition-colors w-[164px] px-2">See Certificate</button> : progress >= 1 ?<button className="text-sm font-semibold text-primary hover:text-[#057834] whitespace-nowrap transition-colors w-[164px] px-2" onClick={onResume}>Resume Course</button> : <button className="text-sm font-semibold text-primary hover:text-[#057834] whitespace-nowrap transition-colors w-[164px] px-2">Start Course</button>}
    </div>
  )
}
