...
interface ModuleFormPanelProps {
  ...
  onCancel: () => void
  onSubmit: (values: { name: string; description: string; lessonCount: string }) => void
}

const ModuleFormPanel: React.FC<ModuleFormPanelProps> = ({
  mode,
  initialName = '',
  initialDescription = '',
  initialLessonCount,
  ...
  const [numberOfLessons, setNumberOfLessons] = useState(
    initialLessonCount !== undefined ? String(initialLessonCount) : ''
  )
  ...
  const handleCancel = () => {
    ...
    setNumberOfLessons(initialLessonCount !== undefined ? String(initialLessonCount) : '')
    onCancel()
  }

  const handleSubmit = () => {
    if (!isValid) return
    onSubmit({ name: moduleName.trim(), description: description.trim(), lessonCount: numberOfLessons })
    // Fields are intentionally left filled after submit, matching the
  ...
  return (
    <div className="rounded-2xl border border-admin-ash-7 bg-white p-5 sm:p-6">
      <h2 className="text-xl font-bold text-admin-ink">
        {isEdit ? 'Edit Module' : 'Add a New Module'}
      </h2>
      <p className="mt-1 text-sm text-admin-ash-3">
        {isEdit
          ? "Update this module's name, description and lesson count."
